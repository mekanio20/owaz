    ...
    <div class="w-full">
        ...
        <div class="container">
            <div class="mt-10 flex items-center space-x-2">
                ...
                <router-link to="/"
                    class=" font-sf_pro font-normal tablet:text-lg text-sm text-m_gray-200">Products</router-link>
            </div>
...
export default {
    ...
    methods: {
        async allProducts() {
            ...
            const category = await api.get(`/category?id=${this.$route.params.id}`)
            this.name = category.rows[0].name_en
